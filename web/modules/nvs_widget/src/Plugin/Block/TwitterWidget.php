...
class TwitterWidget extends BlockBase {
  ...
  public function getTweets(){
    ...
    $tweetout = '<ul class="footer-twitter-feed-vertical__list">';
    foreach ($my_tweets as $key => $value) {
      if(isset($my_tweets->errors)){           
          $tweetout .= '<li class="nodays-error">Error :'. $my_tweets->errors[0]->code. ' - '. $my_tweets->errors[0]->message.'</li>';
      }else{
//        print_r($value);
        //profile_image_url
        ...
        //output custom here
        $tweetout .= '<li class="footer-twitter-feed-vertical__item ico-119"><i class="fa fa-twitter"></i>'.' <a  href="http://twitter.com/'.$value->user->name.'"> @'.$value->user->name.'</a> '.$this->makeClickableLinks($status.'<div class ="date"> '.$this->twitter_time($created)).'</div></li>';
